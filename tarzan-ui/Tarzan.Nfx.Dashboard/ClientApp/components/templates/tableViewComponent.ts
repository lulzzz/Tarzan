...
export class TableViewComponent<ViewType> extends Vue {
    ...
    protected fetch() {
        fetch(this.fetchPrefix + 'count').then(response => response.text().then(value => this.totalItems = parseInt(value)));
        this.reload(1);
    }
    ...
    protected reload(page: number) {
        ...
        let fetchString = this.fetchPrefix + `range/${offset}/count/${this.perPage}`;
